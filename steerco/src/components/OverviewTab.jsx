...
const COLORS = ['#8A05BE','#E0002A','#D48000','#1A6FCC','#007A57','#9B0020','#5C00A3']
...
export default function OverviewTab({ issues, aps }) {
  ...
  // ── Issues por Business Area ──────────────────────────────────────────────
  const baIssues = {}
  issues.forEach(i => {
    const ba = i['Business Area'] || 'Unknown'
    if (!baIssues[ba]) baIssues[ba] = { ba, 'On Track':0, Late:0, TBD:0, total:0 }
    const s = i.status === 'Late' ? 'Late' : i.status === 'TBD' ? 'TBD' : 'On Track'
    baIssues[ba][s]++
    baIssues[ba].total++
  })
  const baIssuesData = Object.values(baIssues).sort((a,b) => b.total - a.total).slice(0, 10)

  // ── APs por Business Area ─────────────────────────────────────────────────
  const baAPs = {}
  aps.forEach(a => {
  ...
  const DONUT_COLORS = { Late:'#E0002A', TBD:'#D48000', 'On Track':'#007A57' }

  // ── Rating breakdown ──────────────────────────────────────────────────────
  const ratingCount = issues.reduce((acc, i) => { acc[i.overall_risk_rating] = (acc[i.overall_risk_rating]||0)+1; return acc }, {})
  const ratingData  = [
  ...
  return (
    <div>
      ...
      {/* ── Row 1: Issues by BA + Donut ── */}
      <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:16, marginBottom:16, alignItems:'start' }}>
        <ChartCard title="Issues & Potential Issues por Business Area" subtitle="Distribuição por status">
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={baIssuesData} layout="vertical" margin={{ left:8, right:24, top:4, bottom:4 }}>
              ...
              <YAxis type="category" dataKey="ba" width={110} tick={{ fontSize:11, fill:'#1A1A2E' }} axisLine={false} tickLine={false} />
        ...
        <ChartCard title="Status dos Issues" subtitle="Visão geral">
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={donutData} cx="50%" cy="50%" innerRadius={60} outerRadius={95} paddingAngle={3} dataKey="value" label={({ name, value }) => `${name}: ${value}`} labelLine={false}>
                {donutData.map((entry) => (
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:16, marginBottom:16, alignItems:'start' }}>
        <ChartCard title="Action Plans por Business Area" subtitle="Distribuição por status">
          <ResponsiveContainer width="100%" height={280}>
        ...
        <ChartCard title="Issues por Rating" subtitle="Distribuição de risco">
          <div style={{ display:'flex', flexDirection:'column', gap:10, padding:'16px 0' }}>
            {ratingData.map(({ name, value, color }) => {
              const pct = issues.length ? Math.round((value / issues.length) * 100) : 0
              return (
                <div key={name}>
                  ...
                  <div style={{ height:8, background:'#F0EDF5', borderRadius:4, overflow:'hidden' }}>
      ...
      {/* ── Business Area Table ── */}
      <ChartCard title="Visão por Business Area" subtitle="Issues + APs consolidados">
        <BATable issues={issues} aps={aps} />
      </ChartCard>
    </div>
  )
}
...
function BATable({ issues, aps }) {
  const bas = [...new Set([...issues.map(i => i['Business Area']), ...aps.map(a => a['Business Area'])].filter(Boolean))].sort()
      ...
      <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
        ...
        <tbody>
          {bas.map((ba, i) => {
            ...
            return (
              <tr key={ba} style={{ background: i%2===0?'#FAFAFA':'#fff', borderBottom:'1px solid rgba(0,0,0,0.04)' }}>
